Shader "Hidden/Shader/UnderwaterPP"
{
    ...
    float4 CustomPostProcess(Varyings input) : SV_Target
    {
        ...
        float2 noiseDirection = normalize(float2(cos(noise * M_PI * 2), sin(noise * M_PI * 2)));
        float2 offsetUv = noiseDirection * _PixelOffset * _PostProcessScreenSize.zw;
        float3 sourceColor = SAMPLE_TEXTURE2D_X(_MainTex, s_linear_clamp_sampler, uv + offsetUv).xyz;
