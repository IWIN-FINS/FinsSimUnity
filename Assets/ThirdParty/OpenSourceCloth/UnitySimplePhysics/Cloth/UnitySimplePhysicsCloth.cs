...
namespace OpenSourceCloth.UnitySimplePhysics
{
    [RequireComponent(typeof(MeshFilter), typeof(MeshRenderer))]
    public class UnitySimplePhysicsCloth : MonoBehaviour
    {
    [Header("RigidBodies")]
    ...
    // Optional non-rectangular topology.  The original sample only created a
    // rows-by-cols sheet; keeping this data here lets the same Rigidbody,
    // ConfigurableJoint, MeshRenderer and MeshCollider path render a genuine
    // triangular cloth rather than a separate visual stand-in.
    private int[] customTriangles;
    private List<Vector2Int> customEdges;

    private LineRenderer[] horizontalLines;
    private LineRenderer[] verticalLines;
    private LineRenderer[] customLines;
    ...
    public void InitTriangular(
        List<Transform> pts,
        int[] surfaceTriangles,
        List<Vector2Int> surfaceEdges,
        ...
        Material clothMaterial)
    {
        ...
        rows = 0;
        cols = 0;
        customTriangles = surfaceTriangles;
        customEdges = surfaceEdges;
        this.lineMaterial = lineMaterial;
        this.clothMaterial = clothMaterial;
    }
    public void Awake()
    ...
    private void CreateLineRenderers()
    {
        if (customEdges != null)
        {
            customLines = new LineRenderer[customEdges.Count];
            for (int i = 0; i < customEdges.Count; i++)
                customLines[i] = CreateLineRenderer($"Edge_{i:000}", 2);
            return;
        }
    ...
    private void UpdateLineRenderers()
    {
        ...
        if (customEdges != null)
        {
            ...
            {
                Vector2Int edge = customEdges[i];
                customLines[i].SetPosition(0, points[edge.x].position);
                customLines[i].SetPosition(1, points[edge.y].position);
            }
            return;
        }
    ...
    {
        bool hasCustomTopology = customTriangles != null && customTriangles.Length >= 3;
        if (!hasCustomTopology && (rows < 2 || cols < 2))
        {
        ...
        int vCount = hasCustomTopology ? points.Count : rows * cols;
        ...
        //Triangles
        if (hasCustomTopology)
        {
            triangles = customTriangles;
            for (int i = 0; i < uvs.Length; i++)
            {
                Vector3 p = points[i].position;
                uvs[i] = new Vector2(p.y, p.z);
            }
        }
        else
        {
    ...
    private void UpdateMeshVertices()
    {
        ...
        if (points == null || points.Count != vertices.Length) return;
