...
namespace FinsSim.Networking
{
    ...
    {
        const string TfVisualizationKey = nameof(Visualizer.FilterValues.Transforms);
        ...
        private void InitializeTf()
        {
            ...
            var visualizer = Visualizer.Instance;
            if (RosConnection.Instance.DisplayTf)
            {
                visualizer.DrawFilter &= ~Visualizer.FilterValues.Transforms;
            }
            else
            {
                visualizer.DrawFilter |= Visualizer.FilterValues.Transforms;
            ...
            visualizer.AddTransform(_mapFrame, TfVisualizationKey);
        }
        ...
        private GameObject GetOrCreateGameObjectForFrame(TfFrame frame)
        {
            ...
            if (obj == null)
            {
                ...
                if (RosConnection.Instance.DisplayTf)
                {
                    Visualizer.Instance.AddTransform(obj.transform, TfVisualizationKey);
                }
            }
            return obj;
        }
